...
class RBM(object):
    ...
    def _create_data_directories(self):
        ...
        models_dir = os.path.join(self.main_dir, 'models')
        data_dir = os.path.join(self.main_dir, 'data')
        summary_dir = os.path.join(self.main_dir, 'summary')
    ...
    def transform(self, data, name='train', save=False):
        ...
        with tf.Session() as self.tf_session:
            ...
            if save:
                np.save(os.path.join(self.data_dir, self.model_name + '-' + name), encoded_data)
            return encoded_data

    def generate(self, data):
        encodings = self.transform(data)
        
        with tf.Session() as self.tf_session:
            self.tf_saver.restore(self.tf_session, self.model_path)
            
            return self.sample_visible_from_hidden(encodings).eval()
